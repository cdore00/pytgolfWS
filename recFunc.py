			...
			if not isinstance(param, (list)) and param.get("user"):
				...
				if param.get("pass") and not isinstance(param, (list)):
					...
					if str(dic['motpass']) == passw:
						...
						res = coll.update({"courriel": user}, { "$set": {"sessID": ""}})
						key = 'sessID'
...
def getUserIdent(o_id):
	...
	if doc.count() > 0:
		data["nom"] = doc[0]["Nom"]
		data["email"] = doc[0]["courriel"]
		data["imgUser"] = doc[0]["imgUser"]
	return data
	...
	try:
		if localHost or checkSession(self, role = ['ADM']):
			if param.get("id"):
				...
				coll = dataBase.users
				docr = coll.update({"_id": o_id}, { "$set": {'Nom': name, 'courriel': user, "niveau": role, "actif": active, "imgUser": imgUser } })
				return dumps(docr)
			else:
				return dumps({'ok': 0})	# No param
		else: 
...
def writeNews(param, self):
	...
	if param:
		...
		if param.get("id") is None:
			docr = coll.insert({"title": param['title'][0] , "active": int(param['active'][0]), "date": param['date'][0] , "dateC": int(time.time() * 1000), "dateM": int(time.time() * 1000), "contentL": param['contentL'][0], "contentR": param['contentR'][0]}, {"new":True})
